...
export async function optimizeReduction(
  motor: MotorDict,
  comLength: MeasurementDict,
  armMass: MeasurementDict,
  currentLimit: MeasurementDict,
  startAngle: MeasurementDict,
  endAngle: MeasurementDict,
  efficiency: number,
  iterationLimit: number,
): Promise<number> {
  let bestReduction = 0;
  ...
  for (let reduction = 1; reduction <= 100; reduction++) {
    const states = await calculateArmStates(
      motor,
      { magnitude: reduction, ratioType: "Reduction" },
      comLength,
      armMass,
      currentLimit,
      startAngle,
